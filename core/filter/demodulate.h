...
public:
  ...
  Demodulate(ImageType &in, const std::vector<size_t> &inner_axes, const bool linear)
    ...
    auto gen_linear_phase =
        [&](Image<value_type> &input, Image<cdouble> &kspace, Image<cfloat> &phase, const std::vector<size_t> &axes) {
          ...
          const pos_type cycle_voxel({axis_mask[0] ? (input.size(0) / kspace_offset[0]) : real_type(0),
                                      axis_mask[1] ? (input.size(1) / kspace_offset[1]) : real_type(0),
                                      axis_mask[2] ? (input.size(2) / kspace_offset[2]) : real_type(0)});
          const pos_type voxel_origin({real_type(0.5 * (input.size(0) - 1.0)),
                                       real_type(0.5 * (input.size(1) - 1.0)),
                                       real_type(0.5 * (input.size(2) - 1.0))});
          for (auto l = Loop(axes)(input, phase); l; ++l) {
            pos = {real_type(input.index(0)), real_type(input.index(1)), real_type(input.index(2))};
  ...
  void operator()(InputImageType &in, OutputImageType &out, const bool forward = false) {
    // if (!out.datatype().is_complex())
    //   throw Exception("Cannot modulate phase for output image that is not complex");
    if (forward) {
  ...
  template <class ImageType> void operator()(ImageType &image, const bool forward = false) {
    // if (!image.datatype().is_complex())
    //   throw Exception("Cannot modulate phase for output image that is not complex");
    if (forward) {
